...
async fn phase10_background_task_nodes_created() {
    ...
    let task_node = Node::new(NodeKind::BackgroundTask, "Task: research JWT")
    ...
    let result_fact = Node::new(NodeKind::Fact, "Task result: research JWT")
        .with_body("Status: completed\n\nJWT best practices: use short expiry times, rotate keys");
    ...
    let e1 = Edge::new(fact_id.clone(), task_id.clone(), EdgeKind::DerivesFrom);
    h.db
    ...
    let e2 = Edge::new(task_id.clone(), session_id.clone(), EdgeKind::PartOf);
    h.db
